...
{
    case 'startInstall':
        Logger::getLogger("AuieoATS")->info("installation start");
        echo '
            <script type="text/javascript">
                setActiveStep(1);
    ...
    case 'installTest':
        Logger::getLogger("AuieoATS")->info("test installation");
        $result = true;
    ...
    case 'databaseConnectivity':
        Logger::getLogger("AuieoATS")->info("database connectivity");
    ...
    case 'mailSettings':
Logger::getLogger("AuieoATS")->info("mail settings");
        ...
        echo '
            <script type="text/javascript">
                ...
                document.getElementById(\'mailSupport\').value = \'opt' . ($mailOption != '' ? $mailOption : urlencode(MAIL_MAILER)) . '\';
                document.getElementById(\'mailSendmail\').value = \'' . urlencode(MAIL_SENDMAIL_PATH) . '\';
                document.getElementById(\'mailSmtpHost\').value = \'' . urlencode(MAIL_SMTP_HOST) . '\';
                document.getElementById(\'mailSmtpPort\').value = \'' . urlencode(MAIL_SMTP_PORT) . '\';
                document.getElementById(\'mailSmtpUsername\').value = \'' . urlencode(MAIL_SMTP_USER) . '\';
                document.getElementById(\'mailSmtpPassword\').value = \'' . urlencode(MAIL_SMTP_PASS) . '\';
                document.getElementById(\'mailFromAddress\').value = \'' . urlencode($mailFromAddress) . '\';
                changeMailForm();
            </script>';
        break;

    case 'setMailSettings':
        Logger::getLogger("AuieoATS")->info("setting mail settings");
    ...
    case 'testDatabaseConnectivity':
        Logger::getLogger("AuieoATS")->info("test database connectivity");
    ...
    case 'resumeParsing':
        Logger::getLogger("AuieoATS")->info("resume parsing");
    ...
    case 'testResumeParsing':
        Logger::getLogger("AuieoATS")->info("test resume parsing");
        echo '
            <script type="text/javascript">
    ...
    case 'testResumeParsing2':
        Logger::getLogger("AuieoATS")->info("test resume parsing2");
        echo '<script type="text/javascript">showTextBlock(\'resumeParsing\');</script>';
    ...
    case 'optionalComponents':
        Logger::getLogger("AuieoATS")->info("initializing optional components");
    ...
    case 'setupOptional':
        Logger::getLogger("AuieoATS")->info("setup optional components");
        initializeOptionalComponents();
    ...
    case 'detectRevision':
Logger::getLogger("AuieoATS")->info("detect revision");
    ...
    case 'queryResetDatabase':
        Logger::getLogger("AuieoATS")->info("query reset database");
        echo '<script type="text/javascript">showTextBlock(\'queryResetDatabase\');</script>';
        break;

    case 'resetDatabase':
Logger::getLogger("AuieoATS")->info("reset database");
        $tables = array();
        $objDB=  DatabaseConnection::getInstance();
        $result = $objDB->getAllRow(sprintf("SHOW TABLES FROM `%s`", DATABASE_NAME));
        if($result)
    ...
    case 'selectDBType':
        Logger::getLogger("AuieoATS")->info("select db type");
        $type = $_REQUEST['type'];
    ...
    case 'restoreFromBackup':
        Logger::getLogger("AuieoATS")->info("restore from backup");
...
Logger::getLogger("AuieoATS")->info("do install empty database");
        ...
        $uiTypeSql="select * from auieo_uitype";
        $result = $objDB->getAllRow($uiTypeSql);
        $arrFieldData=array();
        if($result)
        {
            foreach ($result as $row)
            {
                $arrFieldData[$row["name"]] = true;
            }
        }
        $arrUITypeData=array();
        $arrUITypeData[]=array("name"=>"name","fieldinfo"=>"2910002","length"=>"100","caption"=>"Name");
        $arrUITypeData[]=array("name"=>"phone","fieldinfo"=>"235840002","length"=>"20","caption"=>"Phone");
        $arrUITypeData[]=array("name"=>"email","fieldinfo"=>"167070002","length"=>"100","caption"=>"EMail");
        $arrUITypeData[]=array("name"=>"number","fieldinfo"=>"491520002","length"=>"11","caption"=>"Number");
        $arrUITypeData[]=array("name"=>"owner","fieldinfo"=>"163840723","length"=>"11","caption"=>"Owner");
        $arrUITypeData[]=array("name"=>"date","fieldinfo"=>"266560641","length"=>"30","caption"=>"Date");
        $arrUITypeData[]=array("name"=>"customdropdown","fieldinfo"=>"999999999","length"=>"0","caption"=>"Custom Drop Down");
        foreach($arrUITypeData as $arrUIData)
        {
            if(!isset($arrFieldData[$arrUIData["name"]]))
            {
                $objDB->query("INSERT INTO `auieo_uitype` (`name`, `fieldinfo`, `length`, `caption`) VALUES ('{$arrUIData["name"]}', {$arrUIData["fieldinfo"]}, {$arrUIData["length"]}, '{$arrUIData["caption"]}');");
            }
        }
        
        if(!isset($tables["tag"]))
        {
            $schema="CREATE TABLE `tag` (
        ...
        if(!isset($tables["tag_entry"]))
        {
          $schema="CREATE TABLE IF NOT EXISTS `tag_entry` (
          `id` int(10) unsigned NOT NULL,
            `site_id` int(10) unsigned DEFAULT NULL,
            `data_item_type_id` int(10) unsigned NOT NULL,
            `tag_id` int(10) unsigned NOT NULL,
            `data_item_type` int(11) NOT NULL DEFAULT '0'
          ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
          MySQLQueryMultiple($schema);
        }
        if(!isset($tables["auieo_resumeimport"]))
        {
          $schema="CREATE TABLE `auieo_resumeimport` (
        ...
        if(!isset($tables["auieo_resumeimport_data"]))
        {
          $schema="CREATE TABLE `auieo_resumeimport_data` (
            `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
          `resumeimport_id` int(10) unsigned DEFAULT NULL,
          ...
          `duplicate` int(11) unsigned DEFAULT '0',
            PRIMARY KEY (`id`)
          ) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;";
          MySQLQueryMultiple($schema);
        }
    ...
    case 'onLoadDemoData':
        Logger::getLogger("AuieoATS")->info("on load demo data");
    ...
    case 'upgradeCats':
        Logger::getLogger("AuieoATS")->info("Starting Upgrade Process");
        $tables = array();
        $objDB=  DatabaseConnection::getInstance();
        $result = $objDB->getAllRow(sprintf("SHOW TABLES FROM %s", DATABASE_NAME));
        foreach ($result as $row)
        ...
        if (!isset($fields['date_available']) && isset($tables['client']))
        {
            Logger::getLogger("AuieoATS")->info("upgrading 0.5.0-0.5.1");
            $schema = file_get_contents('db/upgrade-0.5.0-0.5.1.sql');
            MySQLQueryMultiple($schema);
        }
        if (!isset($tables['candidate_joborder_status']) && isset($tables['client']))
        {
            Logger::getLogger("AuieoATS")->info("upgrading 0.5.2-0.5.5");
            $schema = file_get_contents('db/upgrade-0.5.2-0.5.5.sql');
            MySQLQueryMultiple($schema);
        }
        if (!isset($tables['candidate_foreign']) && isset($tables['client']))
        {
            Logger::getLogger("AuieoATS")->info("upgrading 0.5.5-0.6.x");
            $schema = file_get_contents('db/upgrade-0.5.5-0.6.x.sql');
            MySQLQueryMultiple($schema);
        }
        if (!isset($tables['history']) && isset($tables['client']))
        {
            Logger::getLogger("AuieoATS")->info("upgrading 0.6.x-0.7.0");
            $schema = file_get_contents('db/upgrade-0.6.x-0.7.0.sql');
            MySQLQueryMultiple($schema);
        }
        if(!isset($fieldsEmail["for_module"]))
        {
            Logger::getLogger("AuieoATS")->info("upgrading 0.7.0-1.3.2");
            $schema = file_get_contents('db/upgrade-0.7.0-1.3.2.sql');
            MySQLQueryMultiple($schema);
        }
        if (!isset($tables['auieo_fields']))
        {
            Logger::getLogger("AuieoATS")->info("upgrading 1.3.2-1.4.0");
            $schema = file_get_contents('db/upgrade-1.3.2-1.4.0.sql');
            MySQLQueryMultiple($schema);
            $schema = file_get_contents('db/auieo_fields.sql');
            MySQLQueryMultiple($schema);
        ...
        {
            Logger::getLogger("AuieoATS")->info("upgrading 1.4.0-2.1.0");
            $schema = file_get_contents('db/upgrade_1.4.0-2.1.0.sql');
            MySQLQueryMultiple($schema);
        }
        $uiTypeSql="select * from auieo_uitype";
        ...
        if(!isset($tables["tag"]))
        {
            Logger::getLogger("AuieoATS")->info("creating tag table");
            $schema="CREATE TABLE `tag` (
            `tag_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
            `tag_parent_id` int(10) unsigned DEFAULT NULL,
            `title` varchar(255) DEFAULT NULL,
        ...
        if(!isset($tables["tag_entry"]))
        {
            Logger::getLogger("AuieoATS")->info("creating tag entry table");
          $schema="CREATE TABLE IF NOT EXISTS `tag_entry` (
          `id` int(10) unsigned NOT NULL,
            `site_id` int(10) unsigned DEFAULT NULL,
            `data_item_type_id` int(10) unsigned NOT NULL,
        ...
        if(!isset($tables["auieo_resumeimport"]))
        {
            Logger::getLogger("AuieoATS")->info("creating auieo resume import");
          $schema="CREATE TABLE `auieo_resumeimport` (
            `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
            `site_id` int(11) unsigned DEFAULT NULL,
            `date_created` timestamp NULL DEFAULT CURRENT_TIMESTAMP,
        ...
        if(!isset($tables["auieo_resumeimport_data"]))
        {
            Logger::getLogger("AuieoATS")->info("creating auieo resume import data");
          $schema="CREATE TABLE `auieo_resumeimport_data` (
            `id` int(10) unsigned NOT NULL AUTO_INCREMENT,
          `resumeimport_id` int(10) unsigned DEFAULT NULL,
            `name` varchar(255) DEFAULT NULL,
        ...
        echo '<script type="text/javascript">Installpage_populate(\'a=resumeParsing\');</script>';
        Logger::getLogger("AuieoATS")->info("Ending Upgrade Process");
        break;

    case 'maint':
        Logger::getLogger("AuieoATS")->info("maint");
        @session_name(CATS_SESSION_NAME);
        session_start();
    ...
    case 'reindexResumes':
        Logger::getLogger("AuieoATS")->info("reindex resumes");
        echo '<script type="text/javascript">
    ...
    case 'onReindexResumes':
        Logger::getLogger("AuieoATS")->info("on reindex resumes");
    ...
    case 'maintComplete':
Logger::getLogger("AuieoATS")->info("maint complete");
    ...
    case 'loginCATS':
Logger::getLogger("AuieoATS")->info("login cats");
