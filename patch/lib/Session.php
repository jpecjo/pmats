...
class CATSSession
{
    ...
    public function processLogin($username, $password, $addToHistory = true)
    {
        ...
        {
            $this->_isLoggedIn = false;
            $this->_loginError = getLangVar("AUIEO_LANG_LOGIN_FAIL_MESSAGE");
        ...
        switch ($loginStatus)
        {
            ...
            case LOGIN_SUCCESS:
                ...
                if(!$rec)
                {
                    $sql="INSERT INTO `auieo_roles` (`rolename`, `site_id`, `parentid`) VALUES ('AUIEO_ROOT', '{$this->_siteID}', '0');";
                    $db->query($sql);
                    $sql="INSERT INTO `auieo_roles` (`rolename`, `site_id`, `parentid`) VALUES ('Administrator', '{$this->_siteID}', '1');";
                    $db->query($sql);
